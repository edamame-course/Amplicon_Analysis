...
### 1.3.1 Assembling paired-end reads.

```
join_paired_ends.py -f C01D01F_sub.fastq -r C01D01R_sub.fastq -o C01D01
```
This will output three files into a directory called `C01D01`. The three files you will see in here are fastqjoin.join.fastq, something1, and something 2. The fastqjoin.join.fastq has all of the reads that the script succesfully managed to merge. We can count the number of reads that were merged by doing the following.
```
grep '@HWI' fastqjoin.join.fastq | wc -l
```
This will count the number of fastq headers there are in the joined paired end files, indicating the number of sequences successfully merged.
...
```
curl -O https://github.com/edamame-course/Amplicon_Analysis/blob/master/resources/Merged_Reads_Script.sh
```
```
curl -O https://github.com/edamame-course/Amplicon_Analysis/blob/master/resources/list.txt
mkdir Merged_Reads
```
